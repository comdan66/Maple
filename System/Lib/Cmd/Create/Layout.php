...
class Layout {
  ...
  private static function setCRUDFocus(&$column1s, &$column2s) {
    ...
    if ($hasFocus) return;
    foreach ($column1s as &$column1) { $column1['focus'] = true; break; }
    if ($hasFocus) return;
    foreach ($column2s as &$column2) { $column2['focus'] = true; break; }
  }
  ...
  public static function validatorConfigCRUD() {
    ...
    if (isset($parentTitle, $parentRouterUri, $parentControllerName, $parentModelName)) {
      ...
      class_exists($parentModelName) || array_push($errors, '父層 Model「' . \Xterm::gray($modelName, true) . '」' . \Xterm::red('不存在') . '！');
    }
  ...
  public static function createConfigCRUD() {
    ...
    $show  = Tool::getTemplate('CRUD' . DIRECTORY_SEPARATOR . 'View' . DIRECTORY_SEPARATOR . 'Edit' . '.template', ['texts' => $texts, 'enable' => $enable, 'images' => $images, 'parent' => $parent, 'modelName' => $modelName, 'textareas' => $textareas, 'controllerName' => $controllerName]);
